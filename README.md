...
**Changes compared to the official realsense2_camera package:**

- Renamed package realsense2_camera to any_realsense2_camera (avoid name conflicts when installing from ppa).
- Renamed namespace realsense2_camera to any_realsense2_camera (as also the auto-generated messages and dynamic reconfigure code will use the package name as namespace).
- Renamed the exported nodelet realsense2_camera/RealSenseNodeFactory to any_realsense2_camera/RealSenseNodeFactory.
...
```bash
roslaunch any_realsense2_camera rs_camera.launch
```
...
```bash
roslaunch any_realsense2_camera rs_rgbd.launch
```
...
```bash
roslaunch any_realsense2_camera rs_aligned_depth.launch
```
...
Here is an example of how to start the camera node and streaming with two cameras using the [rs_multiple_devices.launch](./any_realsense2_camera/launch/rs_multiple_devices.launch).
```bash
roslaunch any_realsense2_camera rs_multiple_devices.launch serial_no_camera1:=<serial number of the first camera> serial_no_camera2:=<serial number of the second camera>
```
